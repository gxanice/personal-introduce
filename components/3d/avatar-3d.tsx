  ...
  const points = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (isDarkTheme) {
        ...
        // 浅色模式下使用非常深的颜色
        colors[i * 3] = 0.0 + Math.random() * 0.1; // R - 极低红色
        colors[i * 3 + 1] = 0.0 + Math.random() * 0.1; // G - 极低绿色
        colors[i * 3 + 2] = 0.2 + Math.random() * 0.2; // B - 较低蓝色，创建深蓝色
      }
    }
  ...
  useFrame((state) => {
    if (ref.current) {
      ...
      ref.current.rotation.z =
        Math.cos(state.clock.getElapsedTime() * 0.2) * 0.03;
